...
def viewMeasurements( M, G, onCloseCallback=None, F=None ):
	...
	if F is None:
		...
		F.addGeometricField(
			G.name, G,
			geometric_field.makeGeometricFieldEvaluatorSparse(G, F.GFD)
			) 
...
def _addText3D(F, name, value, unit, mOrigin, offset):
	...
	F.scene.mlab.text(textOrigin[0], textOrigin[1], mStr, z=textOrigin[2], width=len(mStr)*charWidth, name='text_'+name)
...
def _drawHead(F,M):
	...
	C = headM.centre
	F.scene.mlab.points3d( C[0], C[1], C[2], mode='sphere', scale_factor=headM.value, resolution=16, name='glyph_headSphere', color=(0.0,1.0,0.0), opacity=0.3 )
...
def _drawNeckWidth(F,M, drawTube=False):
	...
	F.scene.mlab.points3d( NWPoints[0], NWPoints[1], NWPoints[2], name='glyph_neckWidthPoints', mode='sphere', scale_factor=5, resolution=16, color=(1.0,0.0,0.0) )
	...
	if drawTube:
		...
		F.scene.mlab.plot3d(neckEnds[0], neckEnds[1], neckEnds[2], name='glyph_neckWidthTube', tube_radius=neckRadiusM.value/2.0, tube_sides=16, color=(0.0,0.0,1.0), opacity=0.3 )

def _drawFemoralAxisLength(F,M):
	FAL = M.measurements['femoral_axis_length']
	H = FAL.headIntercept[1]
	G = FAL.gTrocIntercept[1]
	F.scene.mlab.points3d( [H[0], G[0]], [H[1], G[1]], [H[2], G[2]], name='glyph_FALPoints', mode='sphere', scale_factor=5, resolution=16, color=(1.0,0.0,0.0) )
...
def _drawSubTrochantericWidth(F, M):
	...
	F.scene.mlab.points3d( points[0], points[1], points[2], name='glyph_sTWPoints', mode='sphere', scale_factor=5, resolution=16, color=(1.0,0.0,0.0) )
...
def _drawMidshaftWidth(F, M, drawTube=False):
	...
	F.scene.mlab.points3d( points[0], points[1], points[2], name='glyph_midshaftWidthPoints', mode='sphere', scale_factor=5, resolution=16, color=(1.0,0.0,0.0) )
	...
	if drawTube:
		...
		F.scene.mlab.plot3d(midshaftEnds[0], midshaftEnds[1], midshaftEnds[2], name='glyph_midshaftWidthTube', tube_radius=mSW.value/2.0, tube_sides=16, color=(0.0,0.0,1.0), opacity=0.3 )
...
def _drawEpicondyleWidth(F,M):
	...
	m = ECW.p2[1]
	c = (l+m)*0.5
	F.scene.mlab.points3d( [l[0], m[0]], [l[1], m[1]], [l[2], m[2]], name='glyph_epicondylarWidthPoints', mode='sphere', scale_factor=5, resolution=16, color=(1.0,0.0,0.0) )
